...
BUILD_ASSERT(
    NRF_DT_GPIOTE_INST(DT_ALIAS(led0), gpios) == GPIOTE_INST,
    "Both sw0 and led0 GPIOs must use the same GPIOTE instance"
);
BUILD_ASSERT(
    IS_ENABLED(_CONCAT(CONFIG_, _CONCAT(NRFX_GPIOTE, GPIOTE_INST))),
    "NRFX_GPIOTE" STRINGIFY(GPIOTE_INST) " must be enabled in Kconfig"
);

static void button_handler(
    nrfx_gpiote_pin_t pin, nrfx_gpiote_trigger_t trigger, void *context
) {
  LOG_INF("GPIO input event callback");
}

int gpiote_test(void) {
  ...
  IRQ_CONNECT(
      DT_IRQN(GPIOTE_NODE), DT_IRQ(GPIOTE_NODE, priority), nrfx_isr,
  ...
  err = nrfx_gpiote_output_configure(
      &gpiote, OUTPUT_PIN, &output_config, &task_config
  );
  if (err != NRFX_SUCCESS) {
  ...
  nrfx_gppi_channel_endpoints_setup(
      ppi_channel, nrfx_gpiote_in_event_address_get(&gpiote, INPUT_PIN),
